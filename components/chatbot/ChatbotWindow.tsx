...
export function ChatbotWindow({ isOpen, messages, isLoading, onSend, translations, direction }: ChatbotWindowProps & { translations?: {
  window: {
    title: string;
    status: string;
    placeholder: string;
  ...
  return (
    <div
      className={cn(
        "fixed bottom-24 left-6 z-[9998] max-w-[90%] w-[400px] h-[600px] max-h-[80vh] flex flex-col bg-background/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-500 ease-in-out",
        isOpen 
          ? "opacity-100 scale-100 translate-y-0 translate-x-0" 
          : "opacity-0 scale-90 translate-y-10 translate-x--10 pointer-events-none"
      )}
    >
      {/* Header */}
